...
//register the model to the model factory with the name RNA16F
RNA16F RNA16F::prototype( "RNA16F" );

RNA16F::RNA16F( const string & registrationName ) :
RNA16D( registrationName ) {
//the private constructor is called by the prototype only
//RNA16F is a restriction of RNA16D
}

RNA16F::RNA16F( ParametersSet & parameters )
: RNA16D( parameters ) {
...
void RNA16F::initialisation( SequenceTable * sequenceTable, int modelId ) {
    //The initialisation procedure for the RNA16F model is slightly different
    //than for the RNA16D model, therefore we redefine it
    ...
    if ( sequenceTable ) {
        ...
        freq4.resize(4);
        //alpha=transition=1.0; beta=transversion; lambda = pair<->MM factor; GU behaviour (=lambda=equivalent(watson-crick))
        rates2.resize(2);
        ...
        //lambda_est^2*kappa*unpair(WC+GU=pair(WC+GU)
        rates2[1] = sqrt( (freq16[0]+freq16[2]+freq16[3]+freq16[5]+freq16[1]+freq16[4])/
                          ( kappa_est * (2*freq4[0]*freq4[3]+2*freq4[1]*freq4[2]+2*freq4[2]*freq4[3]) ) );
...
string RNA16F::getName( void ) const {
    return ( "RNA16F" + MatrixModel::getName() );
}


double RNA16F::getExchangeability( unsigned int residue1,
    ...
    //div by lambda for pair<->GU and GU<->MM ?
    if (index & 0x02){
    ...
    //if pair<->GU this result in division by lambda^2 which is what we wanted
...
double RNA16F::getFrequency( unsigned int residue,
    ...
    switch (residue){
        ...
        case 1 : return ( kappa*lambda2 * (*frequencies)[rateCategory][2] * (*frequencies)[rateCategory][3] ); break;
        ...
        case 4 : return ( kappa*lambda2 * (*frequencies)[rateCategory][3] * (*frequencies)[rateCategory][2] ); break;
...
double RNA16F::getInvKappa( unsigned int rateCategory ) const {
    ...
    invKappa *= ( ((*frequencies)[rateCategory][0] * (*frequencies)[rateCategory][3]) +
               ...
               ((*frequencies)[rateCategory][2] * (*frequencies)[rateCategory][3]) );
    invKappa += 1.0;
    return invKappa;
}
