...
//register the model to the model factory with the name HKY85
HKY85 HKY85::prototype( "HKY85" );

HKY85::HKY85( const string & registrationName ) : DnaModel( registrationName ){
//the private constructor is called by the prototype only
}

HKY85::HKY85( ParametersSet & parameters ) : DnaModel( parameters ){
//the normal constructor is called by the prototype's clone method
}

void HKY85::initialisation( SequenceTable * sequenceTable, int modelId ) {
...
string HKY85::getName( void ) const {
    return ( "HKY85" + MatrixModel::getName() );
}

void HKY85::initMatrixIndex() {
...
void HKY85::setEigenMatrix() {
    ...
    category++ ) {
        ...
        substitutionRate[category] * (*frequencies)(freqCat)[3]; // C->U
        ...
        substitutionRate[category] * (*frequencies)(freqCat)[1]; // U->C
        ...
        -transitionRate * pyrimidineFreq - transversionRate * purineFreq;
        eigenValues[category] [2] =
        -transitionRate * purineFreq - transversionRate * pyrimidineFreq;
        ...
        eigenMatrix[category] ( 3, 0 ) =
        -( (*frequencies)(freqCat)[0] * pyrimidineFreq ) / purineFreq;
        eigenMatrix[category] ( 3, 1 ) = (*frequencies)(freqCat)[1];
        eigenMatrix[category] ( 3, 2 ) =
        -( (*frequencies)(freqCat)[2] * pyrimidineFreq ) / purineFreq;
