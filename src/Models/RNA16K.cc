...
//register the model to the model factory with the name RNA16K
RNA16K RNA16K::prototype( "RNA16K" );

RNA16K::RNA16K( const string & registrationName ) :
RNA16( registrationName ) {
//the private constructor is called by the prototype only
//RNA16K is a restriction of RNA16
}

RNA16K::RNA16K( ParametersSet & parameters )
: RNA16( parameters ) {
...
void RNA16K::initialisation( SequenceTable * sequenceTable, int modelId ) {
    //The initialisation procedure for the RNA16K model is slightly different
    ...
    if ( sequenceTable ) {
        ...
        double ref = ( rates16( 0, 1 ) + rates16( 2, 8 ) + rates16( 3, 4 ) +
                      ...
                      rates16( 13, 15 )+ rates16( 14, 15 ) ) / 2.0;
        // transversion rate ratio
        rate1[0] = ( rates16( 0, 13 ) + rates16( 2, 9 ) + rates16( 3, 14 ) +
...
string RNA16K::getName( void ) const {
    return ( "RNA16K" + MatrixModel::getName() );
}



void RNA16K::initMatrixIndex() {
